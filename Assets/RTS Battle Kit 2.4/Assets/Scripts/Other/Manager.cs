...
public class Manager : MonoBehaviour {
    ...
    void Update() {
        ...
        // Battle music flags
        float playerPercent = playerCastleStrength / playerCastleStrengthStart;
        float enemyPercent = enemyCastleStrength / enemyCastleStrengthStart;

        if (playerPercent > 0.7 && enemyPercent > 0.7)
        {
            fmodDemo.setIsReEnter(true);
        }
        else if (playerPercent > 0.5 || enemyPercent > 0.5)
        {
            fmodDemo.setIsReEnter(false);

            fmodDemo.setDoRepeatFight(true);
        }
        else if (playerPercent > 0.4 || enemyPercent > 0.4)
        {
            Debug.Log("nearly end");
            fmodDemo.setDoRepeatFight(false);
            fmodDemo.setIsNearlyEnd(true);
        }
        else if (playerPercent > 0.3 || enemyPercent > 0.3)
        {
            fmodDemo.setDoRepeatFight(false);
            fmodDemo.setIsNearlyEnd(true);
            fmodDemo.setIsBattleEnd(true);
        }
        else
        {
            fmodDemo.setDoRepeatFight(false);
		...
		if(gameOver && !GameOverMenu.activeSelf){
            ...
            fmodDemo.setIsVictory(false);
            fmodDemo.startBattleResultAudio();
        }

		if(victory && !VictoryMenu.activeSelf){
		    GamePanel.SetActive(false);
		    VictoryMenu.SetActive(true);
            fmodDemo.setIsVictory(true);
            fmodDemo.startBattleResultAudio();
        }
	...
	public void startGame(){
        ...
        // start weather audio
        fmodDemo.startWeatherAudio();
        fmodDemo.startBattleMusic();
	}
	...
	public void endGame(){
        //end game
        fmodDemo.stopWeatherAudio();
        fmodDemo.stopBattleMusic();
        Application.Quit();
    }
	...
	public void restart(){
        fmodDemo.stopBattleResultAudio();
