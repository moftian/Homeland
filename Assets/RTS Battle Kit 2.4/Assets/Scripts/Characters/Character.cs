...
public class Character : MonoBehaviour {
    ...
    FMOD.Studio.EventInstance horseChargingInstance;
    FMOD.Studio.EventInstance horseWhinnyInstance;
    FMOD.Studio.EventInstance yellInstance;
    ...
    void Start() {
        ...
        footstepsInstance = FMODUnity.RuntimeManager.CreateInstance("event:/SFX/Fighting/SoldierFootsteps");
        FMODUnity.RuntimeManager.AttachInstanceToGameObject(footstepsInstance, transform, GetComponent<Rigidbody>());
        yellInstance = FMODUnity.RuntimeManager.CreateInstance("event:/SFX/Fighting/Yell");
        FMODUnity.RuntimeManager.AttachInstanceToGameObject(yellInstance, transform, GetComponent<Rigidbody>());
        horseChargingInstance = FMODUnity.RuntimeManager.CreateInstance("event:/SFX/Fighting/HorseCharging");
        horseWhinnyInstance = FMODUnity.RuntimeManager.CreateInstance("event:/SFX/Fighting/HorseWhinny");

        FMODUnity.RuntimeManager.AttachInstanceToGameObject(horseWhinnyInstance, transform, GetComponent<Rigidbody>());
        FMODUnity.RuntimeManager.AttachInstanceToGameObject(horseChargingInstance, transform, GetComponent<Rigidbody>());

        isAttacking = false;
        isYelling = false;
        isCav = GetComponent<mycav>() ? true : false;
        ...
        if (!isCav)
        {
            ...
            if (agent.speed < 2)
            {
                footstepsInstance.setParameterByName("mySpeed", 2);
            }
            else if (agent.speed >= 2.5)
            {
                footstepsInstance.setParameterByName("mySpeed", 8);
            }

            footstepsInstance.start();
            footstepsInstance.setVolume(1.5f);
        } else
        {
            horseChargingInstance.setVolume(0);
            horseChargingInstance.start();
        }
    }
    ...
    void Update() {
        ...
        if (agent.isStopped)
        {
            footstepsInstance.setPaused(true);
        }
        else
        {
            footstepsInstance.setPaused(false);
            if (isAttacking)
            {
                horseChargingInstance.setPaused(true);
                horseChargingInstance.setVolume(1.5f);
            }
            else
            {
                horseChargingInstance.setPaused(false);
                horseChargingInstance.setVolume(1.5f);
            }
        }
    ...
    public IEnumerator die() {
        FMODUnity.RuntimeManager.PlayOneShot("event:/SFX/Fighting/Death", transform.position);
    ...
    IEnumerator yell()
    {
        if (!GetComponent<archer>())
        {
            if (!isYelling)
            {
                Debug.Log("yelling");
                isYelling = true;
                yellInstance.setParameterByName("quadStrength", 0.8f);
                yellInstance.setVolume(2);
                yellInstance.start();
                if (isCav)
                    horseWhinnyInstance.start();

                //wait and set shooting back to false
                yield return new WaitForSeconds(2.0f);
                yellInstance.stop(FMOD.Studio.STOP_MODE.IMMEDIATE);
                if (isCav)
                    horseWhinnyInstance.stop(FMOD.Studio.STOP_MODE.IMMEDIATE);
                yield return new WaitForSeconds(2.0f);
                isYelling = false;
            }
        }
    ...
    IEnumerator playAttackAudio()
    {
        if (!isAttacking)
        {
            Debug.Log("attack ...");
            isAttacking = true;
            FMODUnity.RuntimeManager.PlayOneShot("event:/SFX/Fighting/Fight", transform.position);
        ...
        horseChargingInstance.setVolume(1);
    }
}
